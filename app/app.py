...
df_story_likes) = (None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None)

(clean_follows, clean_contacts, df_media_prep, df_link_history_prep, df_locations_of_interest_prep, df_last_known_location, df_devices_prep, df_time_spent_on_ig_prep, messages_sent, 
 messages_received) = (None, None, None, None, None, None, None, None, None, None)


# --- Helper function to check if data exists ---
def check_data_exists():
    """Check if Instagram data folder exists and contains data"""
    if DATA_PATH is None:
        return False
    data_path = Path(DATA_PATH)
    if not data_path.exists():
        return False
    # Check if there are at least some key folders/files
    key_folders = ['connections', 'personal_information', 'your_instagram_activity']
    return any((data_path / folder).exists() for folder in key_folders)

def show_upload_prompt(tab_name: str):
    """Display upload prompt for tabs when no data is loaded"""
    st.markdown("---")
    st.header(f"📤 Upload Your Instagram Data")
    st.subheader(f"Explore what Meta knows about you in the **{tab_name}** section")
    
    st.markdown("""
    To visualize your personal Instagram data, you need to first download it from Meta 
    and upload it here. All processing happens **locally on your machine** - your data never leaves your computer.
    """)
    
    st.markdown("### 📁 Upload your data folder")
    
    # File uploader for ZIP file
    uploaded_file = st.file_uploader(
        "Drag and drop your Instagram data ZIP file here",
        type=['zip'],
        help="Upload the ZIP file you downloaded from Instagram Account Center",
        key=f"uploader_{tab_name}"
    )
    
    if uploaded_file is not None:
        if st.button("📥 Import Data", type="primary", key=f"import_{tab_name}"):
            with st.spinner("Extracting and importing your data..."):
                try:
                    ...
                    st.success("✅ Data imported successfully! Please refresh the page to load your data.")
                    st.balloons()
                    st.rerun()
    ...
    1. Go to your Instagram Account Center
    2. Navigate to **Your information and permissions** → **Download your information**
    3. Select your Instagram account and request a download
    4. Wait for the email from Meta with your data (can take up to 48 hours)
    5. Download the ZIP file and upload it here
    """)
    
    st.markdown(
        "🔗 **[Download your data from Instagram Account Center]"
        "(https://accountscenter.instagram.com/info_and_permissions/dyi/?theme=dark)**"
    )
    
    st.caption("💬 For any issues or questions, please contact the project maintainers or open an issue on the GitHub repository.")
...
with st.sidebar:
    ...
    if data_loaded:
        ...
        st.info("📂 No data loaded yet. Upload your Instagram data to enable filters.")
        date_range = []
...
with home:
    ...
    if data_loaded:
        ...
        # Show upload prompt on home when no data
        show_upload_prompt("Home")

with connections_tab:
    if not data_loaded:
        show_upload_prompt("Connections")
    else:
...
with activity_tab:
    if not data_loaded:
        show_upload_prompt("Your Activity")
    else:
...
with preferences_tab:
    if not data_loaded:
        show_upload_prompt("Preferences")
    else:
...
with personal_info_tab:
    if not data_loaded:
        show_upload_prompt("Personal Information")
    else :
